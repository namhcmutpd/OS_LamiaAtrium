...
int __alloc(struct pcb_t *caller, int vmaid, int rgid, addr_t size, addr_t *alloc_addr)
{
  ...
  int freearea_ret = get_free_vmrg_area(caller, vmaid, size, &rgnode);
  if (freearea_ret == 0)
  {
...
#else
  inc_vma_limit(caller, vmaid, PAGING_PAGE_ALIGNSZ(size));
#endif
...
int pg_getpage(struct mm_struct *mm, int pgn, int *fpn, struct pcb_t *caller)
{
  ...
  { /* Page is not online, make it actively living */
    STATS_INC_PAGE_FAULT();  /* Track page fault */
    ...
    STATS_INC_SWAP_OUT();  /* Track swap out */

    /* Update victim page table entry - mark as swapped */
    pte_set_swap(caller, vicpgn, 0, swpfpn);

    /* Check if current page is swapped */
    if (pte & PAGING_PTE_SWAPPED_MASK)
    {
      /* Page is in swap, bring it back */
      addr_t swpoff = PAGING_SWP(pte);
      
      /* Copy from swap to victim's old frame (now free) */
      __swap_cp_page(caller->krnl->active_mswp, swpoff, caller->krnl->mram, vicfpn);
      STATS_INC_SWAP_IN();  /* Track swap in */
      
      /* Return the swap frame */
      MEMPHY_put_freefp(caller->krnl->active_mswp, swpoff);
      
      /* Update current page to point to the frame */
      pte_set_fpn(caller, pgn, vicfpn);
    }
    else
    {
      /* Page was never allocated, just use the victim frame */
      pte_set_fpn(caller, pgn, vicfpn);
    }
...
int pg_getval(struct mm_struct *mm, int addr, BYTE *data, struct pcb_t *caller)
{
  ...
  /* Read from physical memory (RAM) */
  MEMPHY_read(caller->krnl->mram, phyaddr, data);
  STATS_INC_MEM_READ();  /* Track memory read */
...
int pg_setval(struct mm_struct *mm, int addr, BYTE value, struct pcb_t *caller)
{
  ...
  /* Write to physical memory (RAM) */
  /* Note: caller should already hold mmvm_lock */
  MEMPHY_write(caller->krnl->mram, phyaddr, value);
  STATS_INC_MEM_WRITE();  /* Track memory write */
  return 0;
}
...
int __read(struct pcb_t *caller, int vmaid, int rgid, addr_t offset, BYTE *data)
{
  ...
  pthread_mutex_lock(&mmvm_lock);
  pg_getval(caller->krnl->mm, cur_vma->vm_start + offset, data, caller);
  pthread_mutex_unlock(&mmvm_lock);
  return 0;
}
...
int find_victim_page(struct mm_struct *mm, addr_t *retpgn)
{
  ...
  /* FIFO page replacement: find the oldest (last) page in the list */
  if (!pg)
  {
    return -1;
  }
  ...
  /* Traverse to find the last element (oldest page) */
